...
**Answer** It's specific by columns of how data is presented. "Character" is processed as names/labels. "Double" is processed as numerical values. "LGL" I believe is used for logical names like "True or False". Using the first ten rows, that's how the comupter is processing all of its 476,915 observations.  
...
**Answer**
The problems are different categories labeled as "character" or not labeled for numerical value. "Expected" is in a weird format of "true and false."
```{r}
...
**Answer** It's asking to specify columns that are false and it's not doing that I think. 
...
**Answer** The "district" field is labeled as a dbl and the "precinct"is labeled as "character." Lastly, mail is a "dbl."
```{r}
...
**Answer**: There are 402,345 counts under "NA." It is implied that there are roughly 400,000 where mail was not sent for a specific candidate. 
...
**Answer** There's only one entry for January 1900. The first 20 suggest that Yadkin County has a lot of older folks registered to vote. It's listed from the oldest registrants.   
```{r}
yadkin_voters %>% 
...
**Answer**
The "confirmation not returned" should be looked into because there are so amny factors why they wouldn't have returned (weather, wrong address, etc.) The "deceased" is something to look into since in class we learned after awhile it flushes out to new people every couple of months. 
```{r}
...
**Answer** It is labeled as a "character." 
```{r}
glimpse(conowingo)
```
...
**Answer** The city, Conmowwingo is spelled differently or it's in all caps. The amount is labeled as a "character" when we need it as a numerical value. I'm also unsure if the cycle "2022" is supposed to be labeled as a "dbl" and not as a "character."
...
**Answer**
It helps because now it no longer starts with a number. The headers have a more consistent naming system. 
...
**Answer**
There's a problem for the function "summarise." I think it's confused about what to summarize because it asked for all contributions. I think "summarise" is usually used to have a final result of like an "ending product".It says the type is a character, so the "amount" might not be labeled right, it needs to be a numerical value. 
```{r}
# cleaning function
...
**Answer** It is 226 contributions. 
```{r}
# cleaning function
total_conowingo <- cleaned_conowingo %>%
  summarise(total_amount = sum(amount))
...
**Answer**
The donor's name is Derrick Hamilton 
```{r}
cleaned_conowingo %>% 
  get_dupes()
```
...
**Answer**
There are now 13 rows. This now means that if two rows have the same name, the data can be collected to one row. 
```{r}
# cleaning function
cleaned_conowingo <- conowingo %>%
  clean_names() %>% 
...
**Answer**
The Zip Codes shortened to 5 digits and there are parameters set in place so the computer knows which five numbers. 
```{r}
# cleaning function
cleaned_conowingo <- conowingo %>%
  clean_names() %>% 
...
There are two misspellings of Conowingo after running this code. It capitalizes and makes sure that all the cities have the same format of capitalization and look.  
...
**Answer**
There's only one misspelling now. 
```{r}
# cleaning function
cleaned_conowingo <- conowingo %>%
  clean_names() %>% 
...
**Answer** There are now zero misspellings. We used the correct spelling as an ultimatum for all spellings under the city category.
```{r}
# cleaning function
cleaned_conowingo <- conowingo %>%
  clean_names() %>% 
