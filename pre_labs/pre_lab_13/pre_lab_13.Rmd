...
**Answer** 
I notice that each column has their own section. We kept the first dataframe in results, the whole chart is counted as "one" and we made sure "r" saved the table form the dataframe "results[[1]]."
```{r}
...
**Answer** 
The numbers refer to the position of the columns, like the way columns are ordered. We have to clean names, rename and then use mutate to change the columns to numeric values. Below the "gsub" is used to subsitutte objects. In this block, anything with a comma is being subsituted with nothing (" "). 
```{r}
...
**Answer** 
I believe there are zero <ul> tags in this HTML source. We filtered to only find <li> tags.When I searched for "ul", the result came up to seven.  
...
**Answer** 
We can see the committe's name starting at line 21. We can read what each "li" tag is. 
...
**Answer**  We're filtering and making sure the "li" columns have names that will produce the list of committees in a easier format. 
