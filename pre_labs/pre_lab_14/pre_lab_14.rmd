...
**Answer**
The unique function will recognize what words are in a list and eliminate any copies of the same word. 
...
**Answer**
Unnest_token makes all texts lowercase into a column. 
...
**Answer**
Yes, it shows words that are used the most in the dataset. The top words are all common: "the, and, to of..." It shows the top 25 worrds. 
...
**Answer** Yes, now we're shown words that are outside of common vocabulary that is needed in a sentence. Most of hte words are relating to covid-19 or health-related concerns. 
...
**Answer**
We created a list from 2022 and we see that covid is not a common word search for. Health is still at the top plus program and funding.
```{r}
unique_words_2021 %>%
  anti_join(stop_words) %>%
  group_by(word) %>%
...
**Task** Run the following code and describe the results.
**Answer**
Now we have more results that have "two words." The top words are still generic health words. 
```{r}
releases %>%
  filter(date < '2022-01-01') %>%
...
**Answer**
We have it filtered with lesser percentages but showing more names of regions like "prince george's" and "baltimore". Project location is the "top" word in this dataframe because we're looking for "two words" only. 
```{r}
releases %>%
  filter(date >= '2022-01-01') %>%
  unnest_tokens(bigram, text, token = "ngrams", n = 2) %>%
...
**Answer**
Some of them can be counter-intuitive like "work." It's labeled as positive but it can also be negative. 
