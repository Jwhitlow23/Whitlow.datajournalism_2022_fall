...
**Answer**
The problem is that there are duplicates of the same recipient with different counts. dc_food needs to be "cleaned."
```{r}
...
**Answer**
1. There's "We the Pizza" and "Pizza We,The" 
2. There's three different Walter Sports 
3. There's "Whole Foods" and "WholeFoods" and a specific store number for Whole Foods
```{r}
cleaned_dc_food %>%
...
**Answer**
Yes, now it shows a side by side comparison between the recipient names and a "cleaned" version. 
```{r}
cleaned_dc_food <- dc_food %>%
