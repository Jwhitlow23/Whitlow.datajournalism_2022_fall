...
**Answer** "chr" as character for first and last names. Amount number is labeled as "dbl" numerals. The name represents people/ individuals sorted by the amount of rent maybe their organization owes. Maybe this data ia also sorted by the addresses of these businesses and a consistent pay.
...
**Answer** 500-600 per day making contributions 
...
**Answer**
The first one showed contributions by dates in order so 4/01, 4/02 etc. The second codeblock is ordered from most contributions to least contributions 
### Task 8
**Task** Run the following codeblock.
```{r}
maryland_winred_contributions %>%
...
Compared to the previous codeblock, what has changed and how did that affect the result?
**Answer** This one shows specific memos and their amount. The code says "descending amount" and a lot of them are ranging from 2  to 25  at the highest.
...
Compared to the previous codeblock, what has changed and how did that affect the result?
**Answer**
I believe the contributions has some sort of math. It looks like is was descending times total amount and addition is somehow apart of this. Possibly, the singular amounts were multiplied by contributions by memo text
### Task 10
...
**Answer** I'm not sure how to sort them in order. I thought if I just clicked the subheadings then it would sort. I can kind of read the code. This table incldues the mean and median. 
...
**Answer**
This one is grouped by cities while the other is not grouped by cities. This one does not include the memo text.
