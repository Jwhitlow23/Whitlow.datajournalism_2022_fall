...
knitr::include_graphics("images/Dancox_firstexample.png")
```
...
MY LINK: https://datawrapper.dwcdn.net/ZDi81/1/ 
...
md_gov_county <- md_gov_county %>%   
  ...
  mutate(moores_percent=(moore/total_votes)*100)
...
Now we can only see Maryland instead of the entire United States. We only have data for Maryland. 
...
**Answer** Describe the changes from the previous map
We can see percentages in a simpler format that's rounded.The graph displays a county's name and not by FIPS code. 
...
https://datawrapper.dwcdn.net/z14Wb/1/