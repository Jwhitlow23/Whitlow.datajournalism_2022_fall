...
**Answer**
There are ten columns: PRECINCT_ID, PRECINCT_NAME, CONGRESS, LEGIS, COUNCIL, SCHOOL, POLLING_ID, SHAPE_LENGTH, SHAPE AREA, AND SHAPE. 
```{r}
...
**Answer**
Below shows the Prince Georege's County precincts and boundaries which will help us locate the different schools based on the "shape." 
```{r}
pg_precincts %>%
  ggplot() +
  geom_sf() +
...
**Answer**
We uploaded the "tigris" which will help us to use "shape" for geolocation. 
```{r}
...
**Answer**This shows a glimpse of the dataframe organized by eighteen columns.
```{r}
...
**Answer** 
We have to see the output using the ggplot. We will only see results with the STATEFP.
```{r}
...
**Answer**
It shows the different sections, it should be 24. 
```{r}
md_counties %>%
  ggplot() +
  geom_sf() +
...
**Answer**Line 172 is a statement saying we want a plot. Line 173 shows maryland counties filtered by the column "CountyFP" and it's an extra layer that goes into detials (first layer). The next line is another layer showing PG's precincts. Line 175 tells us the theme of the graph.
...
#Yes I do notice the differences in boundaries 
...
**Answer** 
We have two new dataframes. One focuses on the primary electioin 22' and the other focuses on PG's voter turnout. To get the pg turnout dataframe, it stemmed from the primary_22 dataframe and was filtered by its county. It is grouped by precinct and the data made formulas to get the total number of different characteristics like polls, early voting, etc.
```{r}
...
**Answer**
We first insert the new name of the dataset, it stems from "pg_precincts." Left_join is the direction of joining data and we list it in order by how data is joined with pg_turnout first. The "c" is used for lists. We let R know what to look out for using the quotation marks at the end.
...
**Answer**
With the joined data, we use function "aes" to have the graph color-coded. We only want total_eligible and the "scale_colour_viridis_b" shows the color we're using.
```{r}
ggplot() +
...
**Answer**
We install the necessary libraries and our census_api_key.
...
**Answer**
We state the name of our dataframe first and say where we're getting the information from using the csv file. We want the data to be arranged by total (largest to smallest amount)
```{r}
...
**Answer**
We are using get_acs to search for a specific variable and population specific in Maryland.
```{r}
...
**Answer**
We added "geometry" to draw a county map that shows trends, a county shapefile.
```{r}
md_county_population <- get_acs(geography = "county",
              variables = c(population = "B01001_001"),
              state = "MD",
...
**Answer**
We are seeing the differences between the two dataframes when it comes to spelling of names that are similar.
```{r}
md_county_population
...
**Answer**
We're cleaning the data, I'm not sure why an error popped up.
```{r}
...
**Answer**
We're creating a new dataframe stemming from the md_county_population. It it joined with the "voters_by_county"
...
**Answer**
We are now using mutate to make sure all the spellings for Saint Mary's and Baltimore County look the same. 
```{r}
md_county_population <- md_county_population %>%
...
**Answer**
In order to get the number of voters per 10,000 it's stemmed from the md_county_population. Next, we join using the voters_by county dataframe first. We used mutate to create another column that calculates every 10,000 voters. Next, it is arranged from highest to lowest.
```{r}
md_voters_per_10k <- md_county_population %>%
  left_join(voters_by_county) %>%
...
**Answer** We created a graph to show the voters per 10,000 voters. The graph is filled by color.
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We changed the graph to have different colors. Now we can see the graph more easily.  
...
**Answer**
The only section that has 8000 is on the far right, the middle section shows a consistent number between 6500-7500. 
```{r}
ggplot() +
  geom_sf(data=md_voters_per_10k, aes(fill=voters_per_10k)) +
  theme_minimal() +
...
**Answer** 
We used mutate to create another section that calculates "una" population by 10,000.
...
**Answer** 
It's mainly the top part of MD (upper) that has high numbers.
```{r}
ggplot() +
