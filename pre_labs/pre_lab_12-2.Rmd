...
**Answer**
It shows the types of ways people voted in in MD in October. It displays the types of registration and parties. 
```{r}
...
**Answer** 
We created a dataframe only focusing on new registrations and it's sorted by the type of ballot and its party. 
```{r}
...
**Answer** 
We renamed the headers to sort by parties and made sure the columns were counted as numerals. This is a separate dataframe for removals. 
```{r}
# make the new dataframe, renaming the columns and ditching the columns we don't want
...
**Answer**
For Democrats, there's a ten percent difference between in-person votes and votes by mail,more people voted via mail than in-person. Democrats had a bigger voter turnout tan Republicans for the month of October.
```{r}
...
**Task** Run the following code and describe what you think are the most interesting percentages in the dataframe.
**Answer**
There is a high percentage of ballots "county-transfer out." It seems that all ballots that were democratic were 100 percent mail verified and the other parties had zero percent. 
```{r}
removals <- removals %>% 
  mutate(pct_dem = DEM/total *100, pct_rep = REP/total *100, pct_una = UNA/total *100)
...
**Answer** 
there are 17 rows for June and 16 for the new dataframe combining October and June. 
```{r}
...
**Answer** 
Now it's fixed back to 17 rows. 
```{r}
new_reg_oct_jun <- new_registrations %>% 
...
**Answer** 
For the democrat's percentage, they loss 50 percent while the republicans gained 50 percent for recruiting. 
```{r}
...
**Answer**
The chart is showing the type of methods people use for voting. There is a high trend in absentee ballots, volunteer, ERIC report, and provisional ballot. 
```{r fig.width=9}
new_reg_oct_jun %>% 
...
**Answer** 
The main difference is that there is now two graphs. One is for June and one is for October. We can now compare the two months. 
```{r fig.width=9}
library(cowplot)
