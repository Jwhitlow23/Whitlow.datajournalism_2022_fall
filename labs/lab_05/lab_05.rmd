...
cleaned_md_expenses
#Making sure the date columns are datatypes: 
...
**A1.**  I know the code is right for "drop.na" but I posted this code right below to see if I needed to use "true." I'm really doubing myself because I don't see a change of observations between md_expenses and cleaned_md_expenses. I've lsited every code I've tried and

cleaned_md_expenses %>%  
+ group_by(payee_name,expense_category) %>% 
+    drop_na(payee_name,expense_category) %>% 
+ arrange(payee_name)
  
  cleaned_md_expenses <- md_expenses %>%
 mutate(payee_name_clean= key_collision_merge(payee_name)) %>%
  select(payee_name, payee_name_clean everything())
  
cleaned_md_expenses <- md_expenses %>%
  mutate(payee_clean=key_collision_merge(payee_name)) %>%
  select(payee_name,payee_clean,everything())
  
 cleaned_md_expenses %>% group_by(payee_name, expense_category) %>% 
   drop_na(payee_name, expense_category = "Media") %>% 
arrange(payee_name)


cleaned_md_expenses
```{r}
#One of them is "Media". Write code to isolate Media expenses where the name of the payee is not NA. - filter on media expenses
...
#Then, using `write_csv`, make a CSV file in your data folder. 
