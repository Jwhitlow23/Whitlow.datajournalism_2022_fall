...
```{r}
utah_results_url<-"https://electionresults.utah.gov/elections/countyCount/451556070"
results<- utah_results_url %>%  
  read_html() %>% 
  html_table()
...
results<- results[[1]] %>%  
  clean_names() %>%  
  rename (county=x,korb=2,mcmullin=3,hansen=4,hamblin=5,seguin=6,lee=7,williams=8) %>% 
slice(-1,-2) %>% 
...
# excel files can have multiple sheets, so look at the file in excel and decide which sheet you want to read.

```{r}
results_2018<- read_xlsx("lab_13/senate_results_2018.xlsx") %>% 
clean_names() %>% 
rename(county=x2018_general_election_results,aalders=u_s_senate,bowden=x3, mccandless=x4,wilson=x5,romney=x6,fitzgerald=x7,reeve=x8,reiksthegn=x9,korb=x10,jackson=x11,judy=x12,jensen=x13 ) %>% 
slice(-1,-2,-32,-33,-34) %>% 
...
```{r}
joined_results<-results%>%left_join(results_2018, by="county")
```
...
```{r}
joined_results<-results%>%left_join(results_2018, by="county") %>% 
  mutate(differenceMC= (mcmullin-wilson)) %>% 
mutate(difference_lee_rom= (lee-romney))
...
At the end, you'll have three dataframes that you want to combine into a single dataframe. When we want to combine the rows of identical dataframes, we used `bind_rows()`. There's an alternative you can use to do the same for combining columns, and it works the same way (bindcols).

When you're done, rename the columns so they make sense, then make sure the date column is an actual date.

Finally, tell me what questions you could ask of this data. Be creative.
#(Dates, titles, URL all on one page), take three data frames and create one dataframe using bindcols which does the same thing as bind_rows.

**A3** 
A great question to ask is to see track monetary funding he announced over the past 1-2 years for community health centers, community college, and community projects. There are twenty-nine results where monetary funding/investment was mentioned.   
```{r}
cardin_senate_url<-"https://www.cardin.senate.gov/?post_type=press-releases"
cardin_results<- cardin_senate_url %>%  
  read_html()
cardin_results
```
...
```{r}
# Put all three into one dataframe using bind_cols 
cardin_results<-bind_cols(dates,titles,url_pages) %>% 
clean_names() %>%  
  rename(date=value_1,titles=value_2,url_pages=value_3) %>% 
