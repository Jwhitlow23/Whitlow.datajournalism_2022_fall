...
```{r}
md_primary_precinctdem<- read_csv("data/md_primary_precinct_dem.csv")
md_primary_precinctrep<- read_csv("data/md_primary_precinct_rep.csv")
```

Next, let's read in the precinct voting location data and make sure you clean up the column names: Problems: caps, and spaces needed 

```{r}
md_primary_precinctrep <- md_primary_precinctrep %>% 
  clean_names()  
md_primary_precinctdem <- md_primary_precinctdem %>%  
  clean_names() 
```
...
```{r}
md_primary_precinct_combined <- bind_rows(list(md_primary_precinctrep, md_primary_precinctdem))
```

## Answer questions
```{r}
votinglocations<- read_csv("data/precinct_voting_locations.csv") 
```
...
A1. I know for order and listing largest to smallest it's arrange(desc(XYZ))
 group_by(candidate) to have a specific candidate?
 I know I have to group the counties to one result 

```{r}
md_primary_precinct_combined %>%  
  filter(candidate_name== "Wes Moore and Aruna Miller")
joined_dataframe <- first_dataframe %>% 
 left_join(second_dataframe, by=c("first_df_column_1"="second_df_column_1", "first_df_column_2"="second_df_column_2"))
arrange(desc())
```
```{r}
 votinglocations_and_mdcombined <- md_primary_precinct_combined %>% 
 left_join(votinglocations, by=c ("election_district_precinct"="PRECINCT_CODE"))   
```
("election_district_precinct"="PRECINCT_CODE", "county"="COUNTY_CODE"))
