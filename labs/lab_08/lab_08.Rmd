...
**A3.** I could not figure out how to make the "NA's" dissappear for councilmanic_district_code. I tried to use the same formula and use "is.character" but it could not find the object. 
 mutate_if(is.numeric,is.character(COUNTY_CODE,COUNTY_NAME,congressional_district_code,legislative_district_code,councilmanic_district_code,Precinct,VOTER_STATUS,GENDER_CODE,PARTY_CODE,EARLY_VOTE_CENTER) ~replace_na(., 0)) %>% 
 %>% 
```{r}
...
filter(age_group=="18-24") %>% 
 ...
 mutate(total_early_casts=( day1 + day2 + day3 + day4 + day5 + day6 + day7 + day8)) 
 



 #group the counties, group the genders, group total ballots with


``` 

```{r}
percent_groups<- early_voting %>%  
  distinct(county_name,gender_code)
